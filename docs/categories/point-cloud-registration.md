# Point Cloud Registration

Point cloud registration, ICP/GICP/NDT, scan matching, feature matching, and correspondence-based alignment.

Repositories: `41`

[Back to README](../../README.md)

- [norlab-ulaval/libpointmatcher](https://github.com/norlab-ulaval/libpointmatcher) - An Iterative Closest Point (ICP) library for 2D and 3D mapping in Robotics
- [koide3/fast_gicp](https://github.com/koide3/fast_gicp) - A collection of GICP-based fast point cloud registration algorithms
- [koide3/hdl_localization](https://github.com/koide3/hdl_localization) - Real-time 3D localization using a (velodyne) 3D LIDAR
- [XuyangBai/awesome-point-cloud-registration](https://github.com/XuyangBai/awesome-point-cloud-registration) - A curated list of point cloud registration.
- [koide3/small_gicp](https://github.com/koide3/small_gicp) - Efficient and parallel algorithms for point cloud registration [C++, Python]
- [chrischoy/FCGF](https://github.com/chrischoy/FCGF) - Fully Convolutional Geometric Features: Fast and accurate 3D features for registration and correspondence.
- [MIT-SPARK/KISS-Matcher](https://github.com/MIT-SPARK/KISS-Matcher) - KISS-Matcher: Fast, Robust, and Scalable Registration + ROS2 SLAM examples
- [liuguitao/AVP-SLAM-PLUS](https://github.com/liuguitao/AVP-SLAM-PLUS) - An implementation of AVP-SLAM and some new contributions
- [yaoyx689/Fast-Robust-ICP](https://github.com/yaoyx689/Fast-Robust-ICP)
- [XiaoBaiiiiii/colmap-pcd](https://github.com/XiaoBaiiiiii/colmap-pcd) - Image-to-point cloud Registration Tool
- [zhangxy0517/3D-Registration-with-Maximal-Cliques](https://github.com/zhangxy0517/3D-Registration-with-Maximal-Cliques) - [CVPR 2023, BSP] 3D Registration with Maximal Cliques
- [url-kaist/Quatro](https://github.com/url-kaist/Quatro) - Fast and robust global registration for terrestrial robots @ ICRA2022
- [koide3/gtsam_points](https://github.com/koide3/gtsam_points) - A collection of GTSAM factors and optimizers for point cloud SLAM
- [QingyongHu/SpinNet](https://github.com/QingyongHu/SpinNet) - [CVPR 2021] SpinNet: Learning a General Surface Descriptor for 3D Point Cloud Registration
- [WHU-USI3DV/FreeReg](https://github.com/WHU-USI3DV/FreeReg) - [ICLR 2024] FreeReg: Image-to-Point Cloud Registration Leveraging Pretrained Diffusion Models and Monocular Depth Estimators
- [XuyangBai/D3Feat](https://github.com/XuyangBai/D3Feat) - [TensorFlow] Official implementation of CVPR'20 oral paper - D3Feat: Joint Learning of Dense Detection and Description of 3D Local Features https://arxiv.org/abs/2003.03164
- [PRBonn/RAP](https://github.com/PRBonn/RAP) - 🎤 Register Any Point: Scaling 3D Point Cloud Registration by Flow Matching
- [uos/rmcl](https://github.com/uos/rmcl) - Mobile Robot Localization in 3D Triangle Meshes & Geometric Scene Graphs
- [aevainc/Doppler-ICP](https://github.com/aevainc/Doppler-ICP) - Official code release for Doppler ICP
- [GradientSpaces/Rectified-Point-Flow](https://github.com/GradientSpaces/Rectified-Point-Flow) - [NeurIPS 2025, Spotlight] Rectified Point Flow: Generic Point Cloud Pose Estimation
- [ai4ce/DeepMapping2](https://github.com/ai4ce/DeepMapping2) - [CVPR2023] DeepMapping2: Self-Supervised Large-Scale LiDAR Map Optimization
- [aserbremen/Multi-Robot-Graph-SLAM](https://github.com/aserbremen/Multi-Robot-Graph-SLAM) - Decentralized Multi-Robot Graph Simultaneous Localization and Mapping
- [XuyangBai/D3Feat.pytorch](https://github.com/XuyangBai/D3Feat.pytorch) - [PyTorch] Official Implementation of CVPR'20 oral paper - D3Feat: Joint Learning of Dense Detection and Description of 3D Local Features https://arxiv.org/abs/2003.03164
- [HKUST-Aerial-Robotics/Pagor](https://github.com/HKUST-Aerial-Robotics/Pagor) - Pyramid Semantic Graph-based Global Point Cloud Registration with Low Overlap (IROS 2023)
- [Forrest-110/FastMAC](https://github.com/Forrest-110/FastMAC) - Source code of CVPR 2024 paper 'FastMAC: Stochastic Spectral Sampling of Correspondence Graph'
- [ispc-lab/HRegNet](https://github.com/ispc-lab/HRegNet) - [ICCV 2021] HRegNet: A Hierarchical Network for Large-scale Outdoor LiDAR Point Cloud Registration
- [bzdfzfer/litamin2](https://github.com/bzdfzfer/litamin2)
- [jinwoolee1230/POLI](https://github.com/jinwoolee1230/POLI) - [RSS 2026] Learning Point Cloud Geometry as a Statistical Manifold: Theory and Practice
- [WHU-USI3DV/MSReg](https://github.com/WHU-USI3DV/MSReg) - [IEEE TGRS 2024] A novel method for registration of MLS and stereo reconstructed point clouds
- [GDUT-Kyle/gicp_slam](https://github.com/GDUT-Kyle/gicp_slam)
- [vniclas/VFM-Registration](https://github.com/vniclas/VFM-Registration) - LiDAR Registration with Visual Foundation Models
- [engcang/nano_gicp](https://github.com/engcang/nano_gicp) - Nano-GICP as a module from the official github repo: [IEEE RA-L & ICRA'22] A lightweight and computationally-efficient frontend LiDAR odometry solution with consistent and accurate locali...
- [NeSC-IV/sage-icp](https://github.com/NeSC-IV/sage-icp) - Semantic Information-Assisted ICP
- [Tars0523/Doppler_Correspondence](https://github.com/Tars0523/Doppler_Correspondence) - [RSS 2025 Award Finalist] Doppler Correspondence
- [mcdermatt/VICET](https://github.com/mcdermatt/VICET) - Velocity corrected Iterative Compact Ellipsoidal Transform
- [Rolin-zrl/MicroG](https://github.com/Rolin-zrl/MicroG) - Micro-Structures Graph-Based Point Cloud Registration for Balancing Efficiency and Accuracy
- [ispc-lab/HRegNet2](https://github.com/ispc-lab/HRegNet2) - Source code of paper: "HRegNet: A Hierarchical Network for Efficient and Accurate Outdoor LiDAR Point Cloud Registration".
- [SLAMWang/MGN-GICP](https://github.com/SLAMWang/MGN-GICP)
- [hyungjinkim0508/gpicp](https://github.com/hyungjinkim0508/gpicp)
- [homtimon/gt_traj_icp_registration](https://github.com/homtimon/gt_traj_icp_registration) - Tool to register point clouds to generate ground truth trajectories
- [mdrwiega/3d_map_server](https://github.com/mdrwiega/3d_map_server)
